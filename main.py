...
def create_output_file(path):
    return os.path.join(os.path.dirname(path),os.path.splitext(os.path.basename(path))[0])

def wrap_command(command,file_path,output_path):
    return command.replace('{file_output}',f'{file_path} -o {output_path}').replace('{file_path}',file_path).replace('{output_file}',output_path)
...
def get_language(args,parser):
    ...
    if args.language is not None:
        ...
        language = full_dict.get(args.language,full_dict.get(form_language(args.language)))
        if language is None:
...
def compile_and_run(args,parser,language,run_args,new=False):
    ...
    if new is False:
        ...
        log(f'Compiling {os.path.basename(file_path)} because there is no existing executable')
    
    if compile_file(args,parser,log):
        cmd = wrap_command(language['run-command'],args.file,output_file)+' '+run_args
...
def run_cmd(cmd,args):
    ...
    try:
        ...
        cl.error(f'Error while running {args.file} process returned code: {e.returncode}')
        sys.exit()
